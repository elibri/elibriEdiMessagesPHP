<?php
  ...
  class EdiOrderResponseTest extends PHPUnit_Framework_TestCase {

     public function load_message($filename) {
       return file_get_contents(dirname(__FILE__) . "/messages/$filename");
     }

     public function testParsingEdiOrderResponse() {
       $json_message = $this->load_message("order_response.json");
       $message = EdiMessage::parse($json_message);
       $this->assertInstanceOf('EdiOrderResponse', $message);
       ...
       $this->assertEquals('1', $message->line_items[0]->quantity);
     ...
     public function testConstructingEdiOrderResponse() {
       $message = new EdiOrderResponse();
       $json_message = $this->load_message("order_response.json");
       $message->order_id = '11';
       $message->order_buyer_number = 'SK/1/2014';
       $message->line_items[] = new EdiLineItem(array('quantity' => '1', 'ean' => '9788388722639', 'buyer_code' => 'SK101', 
